...
exports.createPaymentLink = async (req, res) => {
    try {
        ...
        const existingLink = await Payment.findOne({ order_id: id });
        if (existingLink && (!existingLink.expiresAt || existingLink.expiresAt > new Date())) {
            return res.status(200).json({
                status: 200,
                message: 'Payment successful',
                data: {
                    order_id: existingLink.order_id,
                    app_trans_id: existingLink.app_trans_id,
        ...
        if (result.data.return_code == 1) {
            ...
            const payment = await paymentLink.save();
        ...
        return res.status(500).json({ status: 500, error: 'Payment failed.' });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ status: 500, error: 'Payment failed.' });
    }
